                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ url('user/search') }}" method="post">
                            ...
                            <div class="row">
                                <div class="col-md-2">
                                    <a href="{{ url('user/info/new') }}" class="btn btn-block btn-primary">Add New</a>
                                </div>
                                <div class="col-md-8">
                            ...
                            <table class="table">
                                ...
                                @foreach($data as $key => $value)
                                    <tr>
                                        ...
                                        <td>{{ $value->email }}</td>
                                        <td>{{ $value->user_level->user_level }}</td>
                                        <td class="text-right">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item" href="{{ url('user/info/'.$value->id) }}">Edit</a>
                                                    <a class="dropdown-item" href="javascript:void(0)" onclick="document.getElementById('hapusData{{ $value->id }}').submit();">Delete</a>
                                                    <form action="{{ url('user/delete/'.$value->id) }}" method="post" id="hapusData{{ $value->id }}">
                                                        {{ csrf_field() }}
                                                        {{ method_field('delete') }}
                                                    </form>
                                                </div>
